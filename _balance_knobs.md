...
## 7. Прокачка карт (движок кат.4) — `managers/balance/forge.py` + `core/ForgeRegistry.py`

Сим-слой ковки (`_upgrade_design.md`). ⚠️ 39.3: калиброваны R1/R2; атакующий
потолок флипают атак.теги, оборонный — Закалка+события (триединство, ниже).

| Рычаг | Значение | Где |
|---|---|---|
| `FORGE_POINTS_PER_ACT` (приток FP/бой по актам) | (1, 2, 3) | `forge.py` |
...
Цена тира = `1+2+3+4+5 = 15 FP`; до 15-го уровня = 45 FP. Приток ~120 FP/забег.
