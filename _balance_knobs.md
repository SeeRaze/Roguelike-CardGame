...
### 8. Ставки (RuleStack, опт-ин сложность, С46) — `core/rules/stakes.py`

Ставка = бандл RuleMod (ограничение + опц. награда), активируется на старте забега
(`stakes=...` в `run_single_run`; дефолт None ⇒ baseline НЕ задет). **Калибровка С46 =
true ascension**: Ставка РОНЯЕТ выживаемость, награда за риск откладывается на слой
Энтропии (не урон в забеге). Идеальный баланс НЕ крутили (классы переделают).
**Находка:** ось «лимит карт» (Аскет) структурно НЕ штраф — тонкая колода = консистентность,
классы с компактным стартером (Воин/Берсерк) её любят. Универсальный штраф — только ось HP.

| Рычаг | Значение | Смысл / где |
|---|---|---|
| `ASCETIC_MAX_CARDS` (лимит «Аскета») | **6** | обрезка стартовой колоды; БЕЗ урон-награды; `stakes.py` |
| `FRAGILE_HP_FRACTION` (доля HP «Хрупкости») | **0.30** | макс. HP × этого на старте; `stakes.py` |
| «Хрупкость» урон-мульт | **1.25** | DAMAGE-мод (частичная комп., не покрывает штраф); `stakes.py` |

📊 A/B (wall, N=40 seed=99, Δ медианы смерти к baseline): Хрупкость −3..−11 у ВСЕХ;
Аскет валит 4/6 (Маг −17.5, Призыв −6.5), Воин/Берсерк стойки (+1.5/+2.5, свойство оси);
обе вместе −5..−24.5.

### 9. Долговой движок (овердрафт энергии, §7, С46) — `core/debt.py`

«Power now, pay later»: энергия уходит в минус при розыгрыше → глубина долга = множитель
урона → конец хода гасит HP-процентом. Срез на ЭНЕРГИИ (примитив обобщается на FP/золото/HP
дёшево). Кривая ЛИНЕЙНАЯ по умолчанию (предсказуемо, безопасно для раннера); экспонента
заложена рубильником `DEBT_CURVE_MODE="exp"` под потолок позже. Активен только при
`player.energy_overdraft` (сим: `run_single_run(debt=True)`; live: будущая Ставка «всё в минус»).

| Рычаг | Значение | Смысл / где |
|---|---|---|
| `DEBT_DAMAGE_PER_POINT` (линейная отдача) | **0.25** | +25% урона за 1 ед. долга энергии; `debt.py` |
| `DEBT_CURVE_MODE` (рубильник кривой) | **"linear"** | "linear" \| "exp" — экспонента под потолок (§7); `debt.py` |
| `DEBT_EXP_RATE` (основание exp) | 0.20 | только при mode="exp": (1+r)^долг; `debt.py` |
| `DEBT_MAX_OVERDRAFT` (пол долга) | **4** | амплитудный гард-рейл: глубже не уйти; `debt.py` |
| `DEBT_HP_INTEREST` (процент гашения) | **2** | HP за 1 ед. долга в конце хода; `debt.py` |

📊 A/B (wall, N=40 seed=99, Δ медианы, debt OFF→ON): размен-движок — Воин +16.5,
Друид +9.5, Берсерк +7, Маг +6 (урон-буст > HP-проценту); Разбойник −7, Призыв −9.5.
Числа ограничены (кап ×2, max 8 HP/ход) — раннер стабилен; калибровка отложена
(классы переделают). baseline-гард зелёный (debt=False дефолт ⇒ флаг не ставится).

### 10. Позиционка 3×3 (фронт/тыл, кейстоун, С47) — `core/positioning.py`

Ось РАНГА на стороне игрока: **полный перехват** (жив фронт → одиночный урон не в тыл).
НЕ балансовая ручка, а СУБСТРАТ — калибровать нечего (класс-redesign впереди). Активна
только при `player.positioning_enabled` (сим: `run_single_run(positioning=True)`; live:
ставится в `GameManager.spawn_procedural_enemy`). Смена строя — Эффект Действия
`[Tactical_Move]` (карта/реликвия/босс), не бесплатная кнопка.

| Рычаг | Значение | Смысл / где |
|---|---|---|
| `DEFAULT_LAYOUT` | **{front:1, back:2}** | дефолт: герой во фронте, союзники в тылу; `positioning.py` |
| `MIRRORED_LAYOUT` | **{front:2, back:1}** | зеркало (Призыватель): саммоны фронт, герой тыл; `positioning.py` |
| `Player.mirrored_layout` | **False** (Summoner: True) | класс-флаг зеркальной раскладки |

📊 A/B (wall, N=40 seed=99): Призыватель OFF 61 / ЗЕРКАЛО 62 (Δ+1) / ДЕФОЛТ герой-фронт
**33** (−28! хилого героя выносит одиночный урон). Воин +0 (без союзников). → зеркало =
верный режим призывного класса. baseline-гард зелёный (positioning=False дефолт).
