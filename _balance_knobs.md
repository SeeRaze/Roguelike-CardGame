...
| `TEMPER_GOLD_COST` (цена Закалки) | **60** ⚠️заглушка | ЗОЛОТО за одну Закалку (С57: FP→золото, ось выживаемости); калибр. свипом 1d; `forge.py` |
...
шаг 8, инертен при `atk_mult=1.0`). **Тема-гейт колоды** (`deck_prefers_sharpen`)
маршрутизирует сток выживаемости по ОСИ: офенс-колода точит урон (Заточка, **FP**,
`ForgePolicy`), оборонная копит Max HP (Закалка, **золото**, `EconomyPolicy` — С57
развела валюты по осям). Маршрутизация — в `runner.py` (нужны обе политики).
Память: `balance-findings-dps-bound`, `economy-axis-trinity`, `economy-arc-plan`.
...
### 9. Долговой движок (овердрафт ресурсов, §7 С46 + §4 С49) — `core/debt.py`

«Power now, pay later»: ресурс уходит в минус → глубина долга = множитель урона → расплата.
Общая pure `_debt_multiplier(depth, mode, per_point, exp_rate)` + обёртки по ресурсу. Кривая
ЛИНЕЙНАЯ по умолчанию; экспонента — рубильником `*_CURVE_MODE="exp"`. Живая активация —
Ставка **«Кровавый Кредит»** (ставит оба флага овердрафта). Сим: оси РАЗВЯЗАНЫ
(`run_single_run(debt=...)` энергия / `hp_debt=...` HP).

**Долг ЭНЕРГИИ** (активен при `player.energy_overdraft`; гашение HP-процентом в конце хода):
...
**Долг HP** (С49, субстрат Берсерка; активен при `player.hp_overdraft`; HP уходит в минус,
смерть на дне; грация/FP — хук класса). Дефолты — ЗАГЛУШКИ (калибровка придёт с классом):

| Рычаг | Значение | Смысл / где |
|---|---|---|
| `HP_DEBT_DAMAGE_PER_POINT` | **0.10** | +10% урона за 1 ед. минуса HP; `debt.py` |
| `HP_DEBT_CURVE_MODE` | **"linear"** | "linear" \| "exp" — рубильник кривой; `debt.py` |
| `HP_DEBT_EXP_RATE` | 0.15 | только при mode="exp"; `debt.py` |
| `HP_DEBT_MAX_OVERDRAFT` (пол HP) | **10** | насколько глубоко HP в минус до смерти; `debt.py` |

📊 A/B энергия (wall, N=40 seed=99, Δ медианы, debt OFF→ON): размен-движок — Воин +16.5,
Друид +9.5, Берсерк +7, Маг +6; Разбойник −7, Призыв −9.5. **HP-долг: A/B медиана
НЕинформативна** — отложенная смерть делает классы со щитом неубиваемыми на глубине («стол»
до max_turns); механика доказана юнит-тестами (`test_hp_debt.py`). baseline-гард зелёный
(оба флага дефолт off ⇒ не ставятся).

### 10. Позиционка — 2D-сетка РАНГ × ЛИНИЯ (кейстоун, С47 v1 → С48 v2) — `core/positioning.py`

2D-решётка на ОБЕИХ сторонах: ось РАНГА (фронт/тыл, **полный перехват** жив фронт →
одиночный урон не в тыл, симметрично игрок↔враг) × ось ЛИНИЙ (лево/центр/право, под
«соседнюю ячейку»/«вертикальный ряд» — `neighbors`/`column`). НЕ балансовая ручка, а
СУБСТРАТ — калибровать нечего (класс-redesign впереди). Активна только при
`player.positioning_enabled` (сим: `run_single_run(positioning=True)`; live:
`GameManager.spawn_procedural_enemy`). Смена строя — Эффект Действия `[Tactical_Move]`.

| Рычаг | Значение | Смысл / где |
|---|---|---|
| `DEFAULT_GRID` | **{front:[C], back:[L,R]}** | дефолт Т: герой ЦЕНТР-фронт, союзники края тыла; `positioning.py` |
| `MIRRORED_GRID` | **{front:[L,R], back:[C]}** | зеркало (Призыватель): саммоны края фронта, герой ЦЕНТР-тыл |
...
📊 A/B v2 (перехват ВРАГА игрок→враг + 2D, OFF vs ON): **ON≈OFF** у всех 6 (Δ0; Summ −0.5
шум) — перехват врага негосязаем для клиров (бот чистит группу в любом порядке) → тактич.
субстрат, не множитель. baseline-гард зелёный (positioning=False дефолт).


### 11. Card Fusion + класс Химик (фундамент С49 → класс С51) — `core/fusion.py`, `core/players/chemist.py`
Чистый class-agnostic механизм слияния двух карт в «Глитч-карту». ИНЕРТЕН (никто не
зовёт), пока класс/реликвия не активирует доступ (`player.fusion_enabled`, как
`positioning_enabled`). Тормоз мощности — РЕСУРС класса (**Реагент**), а не цена
розыгрыша. Этап 2 (С51): класс **Химик** на этом механизме — слияние руки
(`CombatManager.fuse_hand_cards`), движок **Нестабильность** (+урон Глитчам/фьюжн).
Спека: `_card_fusion_design.md` v0.3. Сим-натив: `ChemistPolicy`, baseline wall20.5/ceil18.5.

| Рычаг | Значение | Смысл / где |
|---|---|---|
| `FUSED_COST_FLOOR` | **1** | пол стоимости Глитча (анти-абуз 0-cost); `fusion.py` |
| стоимость Глитча | **max(a,b) пол 1** | не выше дороже источника → всегда играбелен; `fused_cost` |
| `MAX_FUSED_EFFECTS` | **6** | гард-рейл цепного фьюжна (стопка эффектов); `can_fuse` |
| `FUSION_REAGENT_COST` | **1** | цена одного слияния в Реагенте; `fusion.py` |
| `REAGENT_PER_TURN` | **1** | фикс-приток Реагента/ход (тормоз темпа); `chemist.py` |
| Нестабильность | **+1/фьюжн** | стак → +N к урону ТОЛЬКО Глитч-карт; `EffectCalc` шаг 2e |

NB калибровки: ceiling(18.5) < wall(20.5) — у Химика пока нет ручного ceiling-ядра
(`CLASS_CORES` в `builds.py`), потолок = только жадный драфт. Калибровка = отдельный шаг.
