...
class Campfire:
    ...
    def draw_screen(view):
        ...
        view.screen.fill((25, 20, 20))  # Теплый фон костра
        mouse_pos = pygame.mouse.get_pos()
        
        # ==============================================================
        # 1. ГЛАВНЫЙ ЭКРАН КОСТРА
        # ==============================================================
        if Campfire.sub_state == "MAIN":
            view.draw_text(f"=== ЭТАЖ {view.gm.current_floor}: У УЮТНОГО КОСТРА ===", view.main_font, WHITE, 100, 100)
            view.draw_text(f"Ваше здоровье: {view.gm.player.hp}/{view.gm.player.max_hp}", view.main_font, GREEN, 100, 160)
        ...
        # ==============================================================
        # 2. ЭКРАН КУЗНИЦЫ (УЛУЧШЕНИЕ КАРТ - ПРОСТОРНАЯ СЕТКА)
        # ==============================================================
        elif Campfire.sub_state == "FORGE":
            view.draw_text("=== КУЗНИЦА: ВЫБЕРИТЕ КАРТУ ДЛЯ УЛУЧШЕНИЯ ===", view.main_font, YELLOW, 100, 50)
            view.draw_text("Крутите колесико мыши для прокрутки колоды:", view.ui_font, WHITE, 100, 95)
            
            # Новое просторное Full HD окно обрезки (от Y=150 до Y=1000) [1]
            ...
            for index, card in enumerate(view.gm.current_deck):
                ...
                # --- ИСПРАВЛЕННАЯ МАТЕМАТИКА FULL HD СЕТКИ ---
                # Шаг 220 пикселей по X, шаг 280 пикселей по Y (с учетом скролла) [1]
                card_x = 100 + col * 220
                ...
                # ХОВЕР-ПОДСВЕТКА В КУЗНИЦЕ: Проверяем, наведена ли мышь прямо сейчас
                is_card_hovered = card_rect.collidepoint(mouse_pos)
                
                # Внутренний сдвиг: если мышь наведена, заставляем карту подпрыгнуть на 10px вверх
                draw_y = card_y - 10 if is_card_hovered else card_y
                
                # Отрисовываем карту через наш единый эталонный метод движка!
                view.draw_card_by_data(card, card_x, draw_y)
                
                # Если карта уже на максимальном уровне, дописываем плашку поверх
                if card.upgraded:
                    view.draw_text("[МАКС]", view.card_desc_font, GREEN, card_rect.x + 15, draw_y + view.card_height - 35)
    ...
    def handle_clicks(view, mouse_pos):
        if Campfire.sub_state == "MAIN":
            if hasattr(view, 'btn_rest_rect') and view.btn_rest_rect.collidepoint(mouse_pos):
                ...
                view.scroll_y = 0  # Сбрасываем скролл перед открытием кузни
