...
class Campfire:
    ...
    def _draw_forge_overlay(screen, font, player, card, card_x, draw_y,
        ...
        C     = Campfire
        # Структурно некуёмые карты (баги + чисто-экшен утилиты без магнитуды) —
        # явная метка вместо цены уровня, чтобы клик не был молчаливым no-op
        # (can_forge их и так блокирует).
        if not forge_mod.card_is_forgeable(card):
