...
class CombatInterface:
    ...
    def _get_intent_damage_color(predicted_dmg, player_shield):
        ...
        if predicted_dmg > player_shield:
            return (255, 51, 51)    # #FF3333 -- пробитие щита
        return (51, 153, 255)       # #3399FF -- щит держит
    ...
    def draw_combat_screen(view):
        ...
        if enemy.hp > 0:
            CombatInterface.draw_hp_bar(view.screen, 100, 120, 500, 25,
                                        enemy.hp, enemy.max_hp, enemy.shield)
            shield_str = f" (+{enemy.shield} Щит)" if enemy.shield > 0 else ""
            view.draw_text(f"HP: {enemy.hp}/{enemy.max_hp}{shield_str}",
                           view.card_font, RED, 100, 155)

            # Динамический предпросмотр урона врага
            if enemy.intent_type == "attack" and enemy.intent_value:
                predicted = EffectCalculator.calculate_damage(
                    attacker=enemy,
                    target=player,
                    base_damage=enemy.intent_value,
                    dry_run=True
                )
                dmg_color = CombatInterface._get_intent_damage_color(predicted, player.shield)
            ...
            view.draw_text(
                f"Статусы: Уязв:{enemy.vulnerable} Слаб:{enemy.weak} "
                f"Мокр:{enemy.wet} Гор:{enemy.ignited}",
                view.ui_font, GRAY, 100, 245
            )
        else:
            view.draw_text(
        ...
        CombatInterface.draw_hp_bar(view.screen, 100, 380, 500, 25,
                                    player.hp, player.max_hp, player.shield)
        player_shield_str = f" (+{player.shield} Щит)" if player.shield > 0 else ""
        view.draw_text(f"HP: {player.hp}/{player.max_hp}{player_shield_str}",
        ...
        view.draw_text(f"Статусы:  Уязв:{player.vulnerable} Слаб:{player.weak}",
                       view.ui_font, GRAY, 100, 510)
        ...
        view.draw_text("КОНЕЦ ХОДА", view.card_font, WHITE,
        ...
        for log_index, message in enumerate(combat.combat_log):
            view.draw_text(message, view.card_desc_font, WHITE,
