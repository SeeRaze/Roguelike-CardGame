...
# Реролл витрины («Обновить», С68): перетасовать 5 карт за золото. Цена растёт за
# каждое нажатие В ЭТОМ визите (сбрасывается при выходе из магазина) + лёгкий рост от
# этажа. Душит бесплатное «фишинг»-рероллинье до идеальной витрины, но даёт управляемый
# сток золота (replayability — рандом всё ещё кусается, реролл не бесплатен).
REROLL_BASE_COST      = 30                # база цены реролла
REROLL_COST_PER_USE   = 30               # +за каждый реролл в этом визите
REROLL_COST_PER_FLOOR = 2                # +за этаж
...
def get_reroll_price(n: int, floor: int) -> int:
    """Цена СЛЕДУЮЩЕГО реролла витрины (С68): база + надбавка за уже сделанные рероллы
    в этом визите + надбавка за этаж. n — число рероллов, уже сделанных в этом визите."""
    return (REROLL_BASE_COST
            + REROLL_COST_PER_USE * max(0, n)
            + REROLL_COST_PER_FLOOR * max(0, floor))
