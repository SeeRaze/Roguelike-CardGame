NEUTRAL_EVENTS = [
    ...
    # ── ИСПЫТАНИЕ (С72): ЗАВЯЗКА цепочки «Марш смерти». Награда СКРЫТА — текст
    # манит и берёт цену (HP), но НЕ спойлерит легендарку. set_flag → развязка
    # «Дедлайн сдан» (special.py) раскроет пейофф, когда нарандомится позже. ──
    {
        "type": "neutral",
        "title": "Кранч",
        "text": (
            "Команда уходит в кранч перед сдачей.\n"
            "Можно подписаться — выжать из себя всё досуха.\n"
            "Обратной дороги уже не будет."
        ),
        "options": [
            {"label": "Подписаться на марш (-20% HP)", "effects": ["set_flag:death_march", "lose_hp_pct:0.20"]},
            {"label": "Поберечь себя",                 "effects": ["skip"]},
        ],
    },
]
