...
class HubView:
    ...
    def handle_click(self, view, mouse_pos):
        ...
        if hasattr(view, 'btn_start_run') and \
            ...
            RunSave.clear_run()
            self.reset()
            # Новая игра = с нуля: пересоздать игрока выбранного класса, стартовую
            # колоду, обнулить экономику/статы забега, RuleStack и per-run баны. БЕЗ
            # этого состояние прошлого забега (золото/реликвии/max_hp/Ставки) текло
            # бы в новый. Путь «Продолжить» (RunSave.restore_run) идёт мимо — цел.
