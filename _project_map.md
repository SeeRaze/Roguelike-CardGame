...
огромный: Друид 76 — Воин/Призыватель 0). Разрешено **добавлять/менять карты и
стартовые колоды классов** (`core/cards/`, `core/players/*.py`, каталог
`core/cards/catalog.py`). Первоочерёдно:
- (а) **Ослабить движок Друида** — «Токсичный взрыв» (бурст + Реген = яд//2) даёт
  слишком много устойчивости (`core/players/abilities/druid.py`).
- (б) **Структурный фикс Призывателя** — приоритет призывов оказался ловушкой,
  союзники обнуляются каждый бой (`allies=[]`). Варианты: персистентность стаи
  между боями / больше HP-защиты / сильнее ранние призывы (число
  `PACK_DAMAGE_PER_ALLY` вторично).
- (в) Воин/Маг тоже в подвале — пересмотреть их способности/колоды.
