...
- **Костёр** (`ui/Campfire.py`, sub_state MAIN/FORGE/SACRIFICE/DRAFT): 5 опций —
  Отдых (хил = 30% недостающего HP, `Creature.rest_heal_amount`), **Кузница**
  (ковка карт за FP: +1 уровень, растущая цена; на майлстоунах 5/10/15 — **драфт
...
- **Регресс-гард баланса** (`managers/balance/baseline.py`): пиннит медианы wall/
  ...
  Доп. метрика `BASELINE_FORGE` (С50): доля forge-ВКЛ ceiling-забегов тройки яруса 1,
  доживших до `FORGE_REACH_FLOOR`=50 — видимость движка ковки (wall/ceiling его прячут;
  у Берсерка потолок ЖИВЁТ в ковке). Гард односторонний (`BASELINE_FORGE_MAX_DROP`=18,
  только обвал движка). См. раздел «Лестница яруса 1».
...
core/players/init.py, base.py, mage.py, rogue.py, warrior.py, druid.py, berserker.py, summoner.py, chemist.py

core/players/ability.py (базовый ClassAbility)

core/players/abilities/init.py, warrior.py, rogue.py, mage.py, druid.py, berserker.py, summoner.py, chemist.py (один файл на способность)
...
- Пассив Берсерка «Ярость крови» (плоский бонус от недостатка HP) — **УБРАН (С49, передел)**. Движок урона Берсерка теперь = HP-долг множитель (шаг 8-ter, только в МИНУСЕ); см. «Долг HP» + «Берсерк "Отрицание Смерти"».
...
- **Долг HP (С49, субстрат Берсерка «Отрицание Смерти»):** `Creature._hp_floor()` =
  ...
  в минусе. Хук-сеам `phases._settle_hp_debt` (NO-OP; **класс Берсерк ПЕРЕОПРЕДЕЛЯЕТ**
  `player.on_hp_debt_settle` — строгая смерть; см. ниже). Всё инертно без флага.
  ВАЖНО (фиксы С49): `end_turn_phase` даёт новый ход при `hp>_hp_floor()` (не `hp>0`) —
  овердрафт-класс в минусе ещё ЖИВ и получает ход; `bot.check_player_defeat` не обнуляет
  hp (смерть детектится `hp>_hp_floor()`). Для обычных классов (пол 0) — байт-в-байт.
...
- **Сигнатурки Берсерка «кровь в мощь» (С55):** `core/cards/berserker.py` — классовые
  карты ОТ пассива (принцип «стартер раскрывает пассив, не закрывает билд»). Три кирпича-
  грани: `DebtScalingDamageEffect` (урон + per_depth×|долг|, движок кат.4 в карте),
  `SelfHarmEffect` (`lose_hp` сквозь щит → нырок в долг; ставится ПЕРЕД ударом),
  `DebtToForgeOnKillEffect` (добивание в долге → часть долга в FP + гашение; остаток берёт
  `on_combat_won`). Карты: Кровавая ярость / Безрассудный удар / Жажда крови (2 в стартдеке,
  3-я в драфт-пуле). Сим: `BerserkerPolicy._class_pick` пилотирует самоурон (None = завершить
  ход, не суицид); `builds._card_score/_card_themes` узнают эффекты (тема `debt`). Эталон
  ceiling 18.5 (честный при пилотировании; дилюция без него → 13). Калибровка чисел — капстоун.
...
- **`core/positioning.py`** (чистый модуль):
  ...
  - **Расстановка** — `assign_party_ranks(player, allies, mirrored)` (+line по Т-схеме, overflow
    стаи циклит линии); `assign_enemy_ranks(enemies)` — **честная симметрия с партией (С51):
    1 фронт + остальные тыл** (1→1Ф · 3→1Ф/2Т · 4→1Ф/3Т; раньше фронт=(n+1)//2 → асимметрия),
    линии `_LINE_FILL_ORDER` ЦЕНТР→ЛЕВО→ПРАВО. Зеркало (саммоны во фронт) — приватно строю
    партии Призывателя, врагов не касается.
...
- **UI** — `panels._draw_rank_chip` (бейдж ФРОНТ/ТЫЛ ·линия на игроке+союзниках+ВРАГАХ) +
  союзники И ВРАГИ по рядам (фронт сверху/тыл снизу), внутри ряда сортировка по линии
  Л→Ц→П; одноряд без рангов / при n=1 (босс — полноразмерная панель). Тело одной панели
  врага в `_draw_enemy_panel_body`, обе раскладки реюзают. ИНВАРИАНТ: `enemy_panel_rects[i]`
  == `enemies[i]` (rect по исходному индексу, хотя рисуем в порядке рангов) → клик-таргетинг
  (`targeting.py`) и рамка цели (`interface.py`) цел.
...
- **BerserkerAbility «Безумие»** (С49, передел): в этот ход карты стоят 0 энергии, но берут HP (стоимость × `MADNESS_HP_PER_COST`=4, сквозь щит → в МИНУС). Проактивный нырок в красную зону под HP-долг множитель. Повторяема каждый ход (лимит = сам HP). Флаг `player.madness_active` (сброс в `start_turn_phase`); ветка цены в `cardplay`. Класс: `hp_overdraft=True`, строгая смерть (`on_hp_debt_settle`: конец хода в минусе без победы → гибель), пик (`on_combat_won`: |минус HP|→FP, hp=1).
...
**Новое событие:** функции в `ui/events/` (positive/negative/neutral/special) + данные в `event_data.py`, эффекты в `event_effects.py`. **HP-ось (С57, шаг 2):** эффект-ключи — `heal_pct`/`lose_hp_pct` (% от max HP), `lose_gold_pct` (% кошелька), `gain_gold_floor` (floor×K, золото аддитивно), `temper_spirit` (+%max HP навсегда). Флэт-ключи (`heal`/`lose_hp`/`gain_gold`/`lose_gold`) живут для совместимости, но в данных НЕ используются (тест-инвариант в `test_event_effects.py`).
...
Реликвии — 33 итого. **Контракт редкости** (рамка калибровки) — module-docstring в
...
- COMMON (13): LuckyClover, SpikedBracelet, ТочильныйКамень, СтараяПиявка, СчастливаяМонетка, ЗасохшийКлевер (Реген 3), Заплатка (+5 max HP), ЗаточенныйОсколок, ПанцирьДикобраза (Шипы 3), ГрозоваяБатарея (Шок 2 врагу), МеткаОхотника (Уязв. 1 врагу), ТотемЯрости (Ярость 1), ФлаконКатализатора (Мокрый случайному врагу)
- UNCOMMON (8): ДревнееОгниво, НамокшаяРукавица, ОкровавленныйШприц, ФлаконСЖелчью, СвинцовыйНабалдашник, ШипастаяБроня, ТрофейныйКлык, СердцеВеликана (+25 max HP при подборе — HP-ось)
- RARE (5): ЭнергоЯдро, СердцеТитана, ГнилойКлык, ЖелезнаяВоля, БерсеркМедальон
- EPIC (5): КоронаВознесения (×1.25 урон/босс), СердцеБездны (+15% макс.HP/босс), ЭхоВечности (Эхо в начале хода), НесокрушимыйБастион (+50% щита как Барьер), КаменьВечнойЖизни (+1% макс.HP в конце КАЖДОГО боя — HP-ось, дрип vs босс-спайки Сердца Бездны)
- LEGENDARY (2): СтеклянныйГлаз (×3 урон, 10% сжечь карту), ГнилоеСердце (max_hp=1 → щит→Сила)
- **Проклятая Корона** (LEGENDARY, урон ×2 / золото→0) — **УБРАНА из выдачи** (С57, 1d-pre): класс жив (`core/relics/advanced/damage.py`), но не в `RELIC_POOL` → заблокирована за будущим ивентом-анлоком (несовместима с осью «золото = выживаемость»).
...
- **Дедуп БЕЗ класса** (С51): ключ = `username/max_floor/kills/max_damage` (НЕ class).
  Один забег приходит локально (с классом) И из сети (эхо POST без класса → «—») —
  раньше class в ключе давал ДУБЛЬ. Класс подтягивается в строку из той копии, что его
  знает. См. `test_save_manager` (дедуп локальный+сетевое эхо).
- **Сеть** (`managers/network_manager.py`): `send_run_record` POST'ит забег при смерти
  (фон-поток, эхо ТОП → кэш); `refresh_leaderboard()` фоновый GET при открытии доски
  (`LeaderboardView.load_data`) — тянет ТОП из таблицы, офлайн-устойчиво (ошибка →
  last-known кэш). Серверная сторона = Google Apps Script в аккаунте юзера
  (`leaderboard_apps_script.gs` в корне = готовый doGet/doPost + инструкция деплоя).
  `server.py` — опц. Flask-мост, НЕ используется (клиент бьёт прямо в Script URL).
- Хаб (`ui/hub/base.py::_draw_meta_stats`) — панель пожизненных статов из `gm.meta`
  («игра помнит тебя»). Инертна без меты/`class_best`.
- ВНЕ объёма (бэклог): мета-валюта, сейв ПОСРЕДИ забега.

## Ярусная прогрессия классов (Сессия 50)
- `core/progression.py` — ЧИСТЫЙ data-driven модуль (без pygame): пирамида 3 ярусов.
  `TIER1=(Warrior, Mage, Berserker)` доступен всегда; `CLASS_TIERS` (вкл. `Demiurge`=3,
  МАЯК — класс ещё не реализован); `is_unlocked(meta, cls)`; `newly_unlocked(meta)`
  (грант по итогам забега + список новооткрытых); `UNLOCK_CONDITIONS` (условие→функция
  меты, **числа = временные заглушки**: Rogue=этаж≥5, Druid=боссов≥1, Summoner=этаж≥6,
  Demiurge=False).
- Персист: `SaveManager._default_meta` ключ `unlocks:[]` (старые сейвы merge-safe);
  `record_run` после `_apply_run` зовёт `newly_unlocked` → грант + возврат новооткрытых.
- UI-гейт: `ui/hub/selectors.py::draw_class_selector` рисует залоченные карточки с
  завесой «ЗАКРЫТО»; `HubView._select_class` отказывает в выборе залоченного.
- **Симулятор/baseline НЕ читают `unlocks`** (классы инстанцируются напрямую, минуя
  UI-гейт) → блокировка влияет только на Хаб, baseline зелёный.
- Каркас под ФИНАЛ: слот `Demiurge` в `CLASS_TIERS`/`UNLOCK_CONDITIONS` = будущий
  анлок-хук за «Идеальный аудит» (см. `demiurge-finale-concept`).

## Лестница яруса 1: Соблюдай / Гни / Ломай (Сессия 50)
Единый хребет тройки яруса 1 — «копи счётчик → конвертируй в мощь» с НАРАСТАЮЩЕЙ
нестабильностью вверх по лестнице. Сеет зёрна Контекстного Окна Демиурга (накопление →
мощь + глитч). Числа стартовые — калибровка тройки = отдельный шаг.
- **Воин «Дисциплина» (Соблюдай):** статус `discipline` (стак, не тикает, в
  `_COMBAT_RESET_KEYS`). Триггер: `Warrior.on_turn_start_passive` — если `shield>0`
  (держал строй; хук зовётся ДО сброса щита, `phases.py`) → +1. Эффект: EffectCalculator
  **шаг 2d** `+discipline` к урону (гейт `is_player_attack`, флат как Мастерство/Сила).
  Защита→дисциплина→атака. Стабильно (чистый плюс). **С56 — «ресурс-как-трата»:**
  сигнатурки СЖИГАЮТ накопленную Дисциплину (`core/cards/warrior.py`):
  `DisciplineBurstDamageEffect` («Карающий строй», Дисц→бурст), `DisciplineToShieldEffect`
  («Стена щитов», Дисц→щит-стена, ось выживаемости), `DisciplineGainEffect` («Стойка»,
  билдер). Трата обнуляет стак ДО `calculate_damage` (без задвоя 2d). Стартер: 2 спендера
  (Стойка/Возмездие/Заслон — в драфт-пуле). Сим: `WarriorPolicy` копит→жжёт. БЕЗ
  FP-генерации (ось выживаемости, не оффенс — `economy-axis-trinity`).
- **Маг «Нестабильность» (Гни):** на пороге `mastery≥MASTERY_INSTABILITY_THRESHOLD`(5)
  бонус Мастерства в **шаге 2c** усиливается ×`MASTERY_INSTABILITY_MULT`(1.5) (перегруз).
  Цена: `Mage.on_turn_start_passive` теряет HP сквозь щит («искрит»). **С56 — %-ЭСКАЛАЦИЯ:**
  цена = `instability_cost(max_hp, mastery)` = base 4% + 1%/стак Мастерства сверх порога,
  от MAX HP (масштаб-инвариантна, чистая функция; зовёт и пассив, и бот-политика). Комбо-
  хил тоже % (8% max HP). Ось класса = **HP/казино** (НЕ FP, [[economy-axis-trinity]]):
  Маг = HP-churn (%-искрит/%-лечится). Сигнатурки (`core/cards/mage.py`): `OverclockEffect`
  («Разгон», заплати %HP → +Мастерство — активный гамбл), `MasteryScalingDamageEffect`
  («Резонансный разряд», урон+Мастерство×k, читает НЕ тратит). Стартер де-рельсен (Закипание/
  ПАР → пул; ПАР собирается из Поджог+Всплеск). Сим: `MagePolicy` (ПАР первично → разряд →
  Разгон-дотолчок). Эталон спущен (HP-ось недостроена): 18/35, forge 78% — подрастёт сам.
- **Берсерк «Отрицание Смерти» (Ломай):** HP-долг (см. раздел долга / `berserker-redesign`).
  Копит минус HP → множитель урона, ценой смерти. Максимум риска.

**Химик «Нестабильность» (ярус 2, С51, Card Fusion):** отдельный движок — статус
`instability` (стак, не тикает, в `_COMBAT_RESET_KEYS`). Триггер: `Chemist.on_fusion`
(зовётся из `CombatManager.fuse_hand_cards`) → +1 за каждое слияние карт за бой. Эффект:
EffectCalculator **шаг 2e** `+instability` к урону ТОЛЬКО Глитч-карт (гейт `is_player_attack`
+ `card.is_fused`). Внутрибоевой компаунд кат.4 (сброс между боями). Тормоз — ресурс
**Реагент** (приток `REAGENT_PER_TURN`/ход). Гейт доступа `fusion_enabled`. Сим-натив:
`ChemistPolicy`. Спека/детали: `_card_fusion_design.md` v0.3, `chemist-implemented-С51`.

### Калибровка тройки (Сессия 50) — ГРАДИЕНТ-лестница
Согласованная форма силы: **стена ↘ вверх по лестнице, потолок ↗ вверх**. Воин (высокая
стена / умеренный потолок) → Маг (середина) → Берсерк (низкая стена / самый высокий
потолок, запертый за рисковым движком ковки). Эталон (forge-off медианы): Воин 28/51,
Маг 18/35 (С56: спущен после де-рельса+HP-гамбла, подрастёт при достройке HP-оси),
Берсерк 7/17.5.
- **Берсерк = гласс-пушка, НЕ ловушка.** Находка: его стена и потолок — ОДНА проблема,
  хрупкость. Движок (`|HP-долг|→FP→прокачка`) компаундит лишь при выживании; класс мрёт
  ~эт.12 → ковка не раскручивается. С билдом+ковкой+компетентным газом **40% забегов до
  эт.50, max 100**. Низкий wall (~7) принят как идентичность «Ломай» (билд-или-смерть);
  базовую живучесть НЕ трогали (рядом Воин/Маг как безопасные ярус-1 пики).
- **Бот-политика Берсерка (sim-only):** «Безумие» = ставка дисперсии. Раньше жалось вслепую
  @HP≥60% слабой колодой → суицид-ловушка, занижавшая класс. Теперь умный газ: нырять
  только при полном буфере (HP≥0.9·max) И добиваемом залпе (Σ HP врагов ≤0.6·max). Живая
  игра не задета (газ жмёт человек). Безумие = ручка дисперсии в руках игрока (покрутно).
- **Forge-ON метрика (`BASELINE_FORGE`):** wall/ceiling — forge-OFF, что СКРЫВАЕТ движок
  Берсерка. Метрика мерит долю forge-ВКЛ ceiling-забегов до эт.50: Воин 90% / Маг 78%
  (надёжный компаунд) vs Берсерк 32% (рисковый движок). Различающий сигнал (медиана прячет
  движок, p90 у всех в капе). Гард односторонний: тревожит только ОБВАЛ движка.
- NB: Воин/Маг уже в градиенте — класс-ручки не трогали. Память:
  `balance-findings-berserker-glasscannon`.
