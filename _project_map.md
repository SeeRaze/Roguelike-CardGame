...
- **Маг «Нестабильность» (Гни):** на пороге `mastery≥MASTERY_INSTABILITY_THRESHOLD`(5)
  ...
  Цена: `Mage.on_turn_start_passive` теряет `INSTABILITY_HP_COST`(3) HP сквозь щит
  («искрит»). Производное состояние (без нового статуса). Искрит = средняя ступень риска.
...
- **Forge-ON метрика (`BASELINE_FORGE`):** wall/ceiling — forge-OFF, что СКРЫВАЕТ движок
  Берсерка. Метрика мерит долю forge-ВКЛ ceiling-забегов до эт.50: Воин 90% / Маг 100%
