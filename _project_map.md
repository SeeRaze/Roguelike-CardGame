...
- `policy.py` (Сессия 34) — `BotPolicy` (одна на класс) + реестр `get_policy`.
  Тайминг классовой способности (`on_turn_begin`/`on_turn_end` по предусловиям:
  щит/стаки/проактивно) + `pick_card` (Призыватель приоритизирует призывы по
  `SummonEffect`). Пороги «компетентности» — именованные константы вверху файла.
...
- ⚠️ **Артефакт симулятора (Сессия 36):** `_maybe_reward_card` берёт награды
  `random.choice` из generic-пула, но у бота НЕТ политики под Шок. После добавления
  3 карт Шока в общий пул глубокие классы просели (Druid 22→~13, Rogue 12→6,
  Berserker 11→6) — бот тащит «Разряд» (0 урона)/«Серию молний» как мусор и
  разбавляет колоду. Это НЕ реальный баланс-баг (живой игрок юзает синергию в плюс),
  а предел модели: бот не пилотирует синергийные карты. Чинить — политикой бота под
  Шок (отложено). См. [[balance-findings-shock-dilution]].
