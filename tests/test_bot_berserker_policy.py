...
# Бот-политика Берсерка (managers/balance/policy.py): «Аврал» = ставка ДИСПЕРСИИ.
...
def _setup(make_combat, make_creature, hp, enemy_hp):
    """Берсерк с заданным HP против врага с заданным HP. overdrive_active сброшен."""
    p = Berserker()
    p.hp = hp
    p.overdrive_active = False
...
def test_газ_не_жмётся_буфер_низкий(make_combat, make_creature):
    ...
    BerserkerPolicy().on_turn_begin(cm)
    assert p.overdrive_active is False
