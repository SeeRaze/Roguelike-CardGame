...
def test_берсерк_стартдек_содержит_три_сигнатурки_учителя():
    ...
    assert "Эскалация" in names            # грань долг→урон
    assert "Переработка" in names          # грань долг→FP
    assert "Кранч" in names                # грань добил-в-долге→второе дыхание (сустейн)
    assert "Форс-пуш" not in names         # 4-я сигнатурка — в драфт-пул, не в стартер
    assert len(deck) == 9                   # состав не раздут (−1 Коммит под Кранч)
    assert names.count("Коммит") == 2       # пол-воркхорс: было 3, уступил один Кранчу
...
def test_маг_стартдек_де_рельсен_хотфикс_не_пред_собран():
    ...
    names = [c.name for c in deck]
    assert "Автопилот" in names                  # гамбл HP → Мастерство
    assert "Сгенерить фичу" in names      # выжать глубину Мастерства
    assert "Залить в прод" not in names           # ХОТФИКС больше НЕ вручён (де-рельс)
    assert "Удачный промпт" not in names  # билдер → драфт-пул
...
def test_закипание_не_срабатывает_хотфикс_само_на_себе(make_combat, make_creature):
    ...
    поэтому Залить в прод НЕ детонирует само-комбо (статусов ещё нет на момент
...
def test_закипание_в_generic_не_в_классе_мага():
    # С57 (чистка под Нестабильность): Залить в прод (чистый ПАР, 0 Мастерства) переехало
...
def test_закипание_в_пуле_выдачи_мага():
    # Залить в прод по-прежнему достижимо Магу — но как generic-карта (в общем пуле).
...
def test_mage_policy_приоритет_энейблера_без_статусов(make_combat, make_creature):
    ...
    assert chosen.name == "Залить в прод"
