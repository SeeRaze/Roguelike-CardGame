...
def test_берсерк_стартдек_содержит_две_сигнатурки_учителя():
    # Стартер раскрывает пассив: 2 сигнатурки граней долга + generic-основа.
    ...
    assert "Безрассудный удар" not in names  # 3-я сигнатурка — в драфт-пул, не в стартер
    assert len(deck) == 9                   # состав не раздут (было 9: −2 strike, +2 сигн.)
    assert names.count("Удар") == 3         # осталось 3 strike (было 5)
...
# ═══════════════════════════════════════════════════════════
# Mage — де-рельсенный стартдек (С56): ПАР собирается, не вручён
# ═══════════════════════════════════════════════════════════

def test_маг_стартдек_де_рельсен_пар_не_пред_собран():
    m = Mage()
    ...
    assert "Закипание" not in names           # ПАР больше НЕ вручён (де-рельс)
    assert "Тайное сосредоточение" not in names  # билдер → драфт-пул
    assert "Поджог" in names and "Всплеск" in names  # половины ПАР раздельно (собери сам)
    assert len(deck) == 9
