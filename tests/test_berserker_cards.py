...
# ═══════════════════════════════════════════════════════════
# DebtPierceDamageEffect — урон с пробитием щита по глубине долга
# ═══════════════════════════════════════════════════════════

def test_пробитие_вне_долга_полностью_блокируется_щитом(make_creature):
    player = _berserker(make_creature)           # hp=60, долга нет → пробитие 0
    enemy = make_creature("Враг", 50, 50)
    enemy.shield = 20
    # base=8, пробитие 0 → весь удар в щит: щит 20−8=12, HP цел.
    DebtPierceDamageEffect(8, 11, 1, 2).execute(player, enemy, None, False)
    assert enemy.shield == 12
    assert enemy.hp == 50


def test_пробитие_в_долге_идёт_сквозь_щит(make_creature):
    player = _berserker(make_creature)
    player.hp = -5                               # глубина долга = 5
    enemy = make_creature("Враг", 50, 50)
    enemy.shield = 20
    # base=8, pierce=min(8, 1×5)=5 сквозь щит (HP 50−5=45), остаток 3 в щит (20−3=17).
    DebtPierceDamageEffect(8, 11, 1, 2).execute(player, enemy, None, False)
    assert enemy.hp == 45
    assert enemy.shield == 17


def test_пробитие_не_превышает_итоговый_урон(make_creature):
    player = _berserker(make_creature)
    player.hp = -50                              # огромный долг → pierce клампится на final
    enemy = make_creature("Враг", 50, 50)
    enemy.shield = 30
    # base=8, pierce=min(8, 1×50)=8 → весь удар сквозь щит, щит цел.
    DebtPierceDamageEffect(8, 11, 1, 2).execute(player, enemy, None, False)
    assert enemy.hp == 42                        # 50−8 (всё сквозь щит)
    assert enemy.shield == 30                    # щит не тронут


def test_пробитие_улучшенное_сильнее_просачивается(make_creature):
    player = _berserker(make_creature)
    player.hp = -3                               # долг 3
    enemy = make_creature("Враг", 50, 50)
    enemy.shield = 20
    # улучш: base=11, pierce=min(11, 2×3)=6 сквозь щит (HP 50−6=44), остаток 5 в щит (15).
    DebtPierceDamageEffect(8, 11, 1, 2).execute(player, enemy, None, True)
    assert enemy.hp == 44
    assert enemy.shield == 15
...
# ═══════════════════════════════════════════════════════════
# Композиция «Переработка»: нырок → удар → (если добил) банк FP
# ═══════════════════════════════════════════════════════════

def test_переработка_композиция_нырок_удар_банк(make_creature):
    ...
    card = Card(
        name="Переработка", cost=1, card_type="attack", description="",
...
def test_берсерк_карты_в_пуле_класса():
    ...
    assert {"Эскалация", "Форс-пуш", "Переработка", "Кранч"} <= pool_names
...
def test_берсерк_карты_не_в_generic():
    ...
    for n in ("Эскалация", "Форс-пуш", "Переработка", "Кранч"):
...
def _bloodthirst():
    return Card("Переработка", 1, "attack", "",
