...
def test_оптимизация_добавляет_урон():
    atk = Creature("Атакующий", 50, 50)
    tgt = Creature("Цель", 50, 50)
    atk.optimize = 3            # +3 к урону
...
def test_порядок_оптимизация_потом_токсичность():
    # Сначала прибавляется оптимизация, потом применяется токсичность.
    atk = Creature("Атакующий", 50, 50)
    tgt = Creature("Цель", 50, 50)
    atk.optimize = 4
    atk.tox = 1
...
def test_комбо_хотфикс_удваивает_урон():
    # «Хотфикс»: цель одновременно под Кофе и Legacy → урон ×2.0. Кофе ДОП. амплифицирует
    # входящий урон (+20%/стак, шаг 4a) ДО комбо: 10 ×1.4 (coffee2) = 14, затем ×2 = 28.
    atk = Creature("Атакующий", 50, 50)
    tgt = Creature("Цель", 50, 50)
    tgt.coffee = 2
    tgt.legacy = 2
    assert calc(atk, tgt, 10) == 28
...
def test_dry_run_не_расходует_стаки_комбо():
    ...
    assert tgt.coffee == 2 and tgt.legacy == 2     # стаки на месте


def test_обычный_расчёт_расходует_стаки_комбо(make_combat):
    # Без dry_run комбо «срабатывает»: по одному стаку Кофе и Legacy уходит,
    ...
    assert tgt.coffee == 1 and tgt.legacy == 1
...
def test_реестр_содержит_правильную_запись_хотфикс():
    hotfix = COMBOS.get("hotfix")
    assert hotfix is not None
    assert hotfix["name"] == "ХОТФИКС"
    assert hotfix["requires"] == ("coffee", "legacy")
    assert hotfix["multiplier"] == 2.0
    assert hotfix["consume"] == 1
    assert "ХОТФИКС" in hotfix["log"]
...
def test_комбо_требует_все_статусы_одновременно(make_combat):
    """Только coffee без legacy → множитель не срабатывает."""
    atk = Creature("Атакующий", 50, 50)
    tgt = Creature("Цель", 50, 50)
    tgt.coffee = 3
    cm = make_combat(player=atk, enemy=tgt)
    # coffee амплифицирует (×1.6), но комбо-множитель НЕ срабатывает (нет legacy).
    assert calc(atk, tgt, 10, combat_manager=cm) == 16
    assert not cm._combo_triggered
