...
# зеркальный флаг (фундамент), смоук opt-in в runner. baseline зелёный (off → no-op).
...
# ═══════════════════════════════════════════════════════════
# Зеркальный флаг (фундамент): инстанс-флаг mirrored_layout
# ═══════════════════════════════════════════════════════════

def test_зеркальный_флаг_инстанса():
    # Производителя-класса нет (С59), но инфра зеркала жива: player с
    # mirrored_layout=True ставит союзников во фронт (см. _apply_positioning).
    from core.players.warrior import Warrior
    p = Warrior()
    p.mirrored_layout = True
    assert getattr(p, "mirrored_layout", False) is True
...
def test_runner_positioning_opt_in_смоук():
    ...
    import random
    random.seed(99)
    res = run_single_run(Warrior, max_floor=5, positioning=True)
