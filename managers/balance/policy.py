...
_WARRIOR_SHIELD_MIN     = 8   # «Откат релиза»: удар = 50% щита
_WARRIOR_RETRIBUTION_MIN = 10 # «Регрессионка»: придержать, пока щит не накоплен
...
_MAGE_OVERCLOCK_HP_FRAC = 0.5 # «Автопилот»: гэмблить HP→Мастерство только при HP ≥ доли max
...
class WarriorPolicy(BotPolicy):
    ...
    щит на конец хода → пассив +1/ход; билдеры «Тест-план» дают сразу), затем СЖИГАЕМ её
    ...
    Старая ось «защита=атака» (Регрессионка/Откат релиза) сохранена как фолбэк."""
    ...
    def _class_pick(self, playable, combat):
        ...
        # 3) Старая ось «Регрессионка» — на накопленном щите.
...
class MagePolicy(BotPolicy):
    ...
    при второй стихии steam ×2 + комбо → +Мастерство/хил/добор). «Автопилот» гэмблит HP →
    Мастерство, чтобы перешагнуть порог ×1.5 — но ТОЛЬКО при безопасном HP (не суицид),
    и лишь пока Мастерство < цели (не транжирить HP впустую). «Сгенерить фичу»
    ...
    def _class_pick(self, playable, combat):
        ...
        player = combat.player
        enemy = combat.enemy
        coffee = getattr(enemy, "coffee", 0) > 0
        legacy = getattr(enemy, "legacy", 0) > 0
        ...
        coffee_cards = [c for c in playable if "coffee" in _status_types(c)]
        legacy_cards = [c for c in playable if "legacy" in _status_types(c)]
        attacks = [c for c in playable if _has_effect(c, DamageEffect)]

        # 1) Собираем ХОТФИКС: нет стихии — вешаем недостающую. Комбо растит Мастерство
        #    БЕСПЛАТНО (+хил/добор) — приоритетнее платного Автопилота (не платим HP за то,
        #    что комбо даёт даром). Карта-атака стихии даёт ещё и hotfix ×2 при второй.
        if not coffee and coffee_cards:
            return coffee_cards[0]
        if not legacy and legacy_cards:
            return legacy_cards[0]
        # 2) ХОТФИКС готов — Сгенерить фичу (payoff глубины) ИЛИ любая атака детонирует.
        if coffee and legacy:
            if discharges:
                return discharges[0]
            detonators = [c for c in attacks
                          if c not in coffee_cards and c not in legacy_cards]
            if detonators:
                return random.choice(detonators)
        # 3) Автопилот — ДОТОЛЧОК к порогу перегруза ×1.5, когда ПАР в этот ход не собрать:
        ...
        if (overclocks and player.mastery < threshold
            ...
            return overclocks[0]
        # 4) Иначе выжимаем накопленное разрядом, затем фолбэк (без Автопилота при низком HP).
        if discharges:
            return discharges[0]
