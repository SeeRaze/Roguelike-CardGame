...
def run_single_run(player_class, max_floor: int = 100, *,
    ...
    Параметры билда (дефолты = метрика WALL, прежнее поведение):
      ...
      positioning — bool (Позиционка §4). True → бот расставляет партию по рангам
                    ...
                    Зеркало берётся из класс-флага mirrored_layout (зеркальный
                    класс → союзники во фронт). По умолчанию False → флаг не
                    ставится, рангов нет → перехват = старый пул (регресс-нейтрально, baseline зелёный).
