...
# ⚠️ ТОЛЬКО ЯРУС-1 (С58). Тир-2 (Химик) НЕ меряем: ему ещё не прописаны механики/
# пассивы под передел классов, числа = мёртвый груз (сотрутся при переделе). База
# баланса и игры в целом строится на тройке яруса-1; тир-2 вернётся в гард при редизайне.
# (Разбойник/Призыватель удалены целиком — С59, как и Друид.)
...
# С67 (Jun 14 2026): РЕ-БЛЕСС под честные живые медианы — баланс яруса-1 ЗАФИКСИРОВАН.
# Числа уплыли от прежнего эталона из-за контент-волн (блоки 2-4) в пределах допуска;
# теперь центры пиннятся к реальности. Сенситивити-проба капстоуна (С67) показала: тройка
# на ПЛОСКОМ плато — TEMPER_GOLD_COST/кривая-врага/крит-Воина почти не двигают метрики →
# значения защитимы, крутить нечего. Допуски (DROP=6/RISE=12) НЕ трогаем: проба показала
# шум wall до ±3 при N=40 (Маг немонотонен) → широкий допуск намеренно его поглощает.
BASELINE = {
    "Warrior":   {"wall": 29,   "ceiling": 48.5},
    "Mage":      {"wall": 13.5, "ceiling": 27},
    "Berserker": {"wall": 7,    "ceiling": 14.5},
}
...
#   • С57 (1d-pre): из ceiling-билдов убрана Проклятая Корона (ныне «Марш смерти», ×2 урон) — она почти в
...
#   • Воин ~42% / Маг ~65% долетают (Маг ниже до достройки HP-оси — HP-гамбл Автопилота
...
BASELINE_FORGE = {
    "Warrior":   85,   # С67 РЕ-БЛЕСС (было 60): контент-волны подняли доходимость до 85 (рост
                       # легитимен). Критический баг МУЛЬТИПЛИКАТИВЕН ×(1+k·Дисц); k=0.30/0.40
                       # ЗАКРЕПЛЁН капстоуном (С67 проба: +0.10 k ≈ +1 ceiling → текущий держит
                       # «умеренный потолок» Воина; задирать k = роль Берсерка, не Воина).
    "Mage":      22,   # С66 РЕ-БЛАГОСЛОВЕНО (было 60 — устаревшая стаб-цель С57 под старого
                       # ПАР-Мага). Диагностика С66: forge-reach Вайб-кодера УПЁРТ В HP-ЭКОНОМИКУ,
                       # НИ ОДНА ручка его не двигает с ~22 — урон ×3 (мимо: бот не доживает до
                       # глубины множителя), глубина перегруза (ХУЖЕ: самоубой, пик Мастерства
                       # застревает на ~8 — HP-цена Автопилота 10%/use запирает разгон), HP-цена
                       # выкл/вдвое (мимо), сустейн в ядро (мимо). Маг = НАСТУПАТЕЛЬНЫЙ HP-гамбл,
                       # а forge-reach награждает ВЫЖИВАЕМОСТЬ → оборонный Воин берёт 60 структурно,
                       # наступательный Маг — нет. Сродни аутопсии Берсерка (бот-/HP-ограничен).
                       # Маг ≠ Берсерк: сила Мага в стене/потолке (16/27 > 8/16), не в доходимости-
                       # вглубь. Лифт к 60 = класс-редизайн HP-экономики (бэклог), не калибровка.
...
#   • Берсерк: ceiling≈wall в стартере. (С66: ядро тройки [escalation/refactoring/
#     crunch] после IT-рескина РАЗЛОЧЕНО — день-1 доступно, в отличие от старого
#     locked [battle_cry]; но эмпирически ceiling всё равно ≈ wall —)
...
BASELINE_STARTER = {
    ...
    # живёт в залоченных реликвиях Кэш/Санитайзер/Оверклокинг).
    "Warrior":   {"wall": 25,   "ceiling": 28},     # С67 ре-блесс (было 25.5/31)
    "Mage":      {"wall": 13.5, "ceiling": 17.5},   # С67 ре-блесс (было 16.5/21)
    "Berserker": {"wall": 6,    "ceiling": 9},      # С67 ре-блесс (было 8/10)
}
