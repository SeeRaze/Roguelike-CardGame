...
class TurnPhaseMixin:
    ...
    def start_turn_phase(self):
        ...
        # КОСТЫЛЬ НА ПРОДЕ (Этап 3): отложенный форс-Аврал — хотфикс спас в фазу врага,
        # пометив _pending_overdrive. Поднимаем Аврал ПОСЛЕ обычного сброса, чтобы он
        # реально действовал в этот ход (firefighting). NO-OP без флага (baseline зелёный).
        if getattr(self.player, '_pending_overdrive', False):
            self.player.overdrive_active = True
            self.player._pending_overdrive = False
            self.add_log_message("[КОСТЫЛЬ НА ПРОДЕ] Форс-Аврал: карты за 0 энергии ценой HP.")
        # E5: пер-ход сброс флага Диспетчера снесён вместе с флагом — Диспетчер теперь
        # даёт Эхо, а заряд Эха живёт до следующей карты (внутрибоевой, как у Ретрая).
        ...
        # _cache_held_shield = снимок щита прошлого хода для реликвии «Кэш».
        self.player._cache_held_shield = self.player.shield
