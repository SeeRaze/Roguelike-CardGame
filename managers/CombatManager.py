...
class CombatManager(
    ...
    # Потолок ПЕРЕНОСА союзников между боями. Лимит на то, сколько выживших
    # уносится в следующий бой — без него стая копилась бы вечно (снежный ком).
    # Враги бьют СЛУЧАЙНУЮ цель (игрок/союзник, см. Enemy._choose_attack_target),
    # поэтому союзники ещё и ТАНКуют — потолок задаёт живучесть партии.
    # (Дремлющий фундамент: производителя-карты призыва нет с С59 — инфра под
    # будущие классы и позиционку 2D-сетки.)
    ...
    def __init__(self, player, enemies, starting_deck, game_manager=None):
        ...
        # ENGINE-множители (С58): Буфер обмена (Ctrl+C/V) — персист-слот на 1 карту,
        # живёт МЕЖДУ ходами (не сбрасывается per-turn). Диспетчер задач — флаг «следующая
        # карта ×2» (одноразовый, гасится в start_turn_phase, если не потрачен).
