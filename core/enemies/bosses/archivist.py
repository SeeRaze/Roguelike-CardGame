...
# Механика: +щит за каждую сыгранную карту + Слабость за толстую колоду.
...
class OblivionArchivist(BossBase):
    ...
    сброс) > 15 — +1 Слабость игроку. В фазе 2 Слабость накладывается всегда.

    Мягкие обходы:
    - Маг: естественно тонкая колода (меньше Shield-накачки)
    - Воин/Берсерк: персистентный щит / Казнь игнорируют вражеский щит
    - Друид: яд тикает сквозь щит
    - Призыватель: союзники танкуют, DPS не снижается от Слабости
    """
    ...
    def on_turn_start(self, player, combat_manager) -> None:
        """Толстая колода (или фаза 2) → Слабость игроку."""
        if combat_manager:
            ...
            if combat_manager:
                if deck_size > self.DECK_SIZE_LIMIT:
                    combat_manager.add_log_message(
                        ...
                        f"ослабляет вас: +1 Слабость."
                    )
                else:
                    combat_manager.add_log_message(
                        "[АРХИВАРИУС] Близость забвения ослабляет вас: +1 Слабость."
                    )
    ...
    def choose_intent(self):
        ...
        if step == 0:
            self.set_intent("debuff", 1)            # +1 Слабость
        elif step == 1:
