...
class ОткатКБэкапу(Relic):
    """В конце боя исцеляет на 20% от недостающего HP."""

    def __init__(self):
        super().__init__(
            "Откат к бэкапу",
            "Откат к стабильной версии: в конце каждого боя восстанавливает 20% недостающего HP.",
            Rarity.RARE,
        )
...
class ФоновоеИндексирование(Relic):
    """В начале боя дает игроку 3 Хелсчека.

    Калибровка: хелсчек убывает на 1 за тик, поэтому Хелсчек 1 лечил суммарно
    лишь 1 HP — ниже порога полезности даже для обычного. 3 = 3+2+1 = 6 HP/бой."""

    def __init__(self):
        super().__init__(
            "Фоновое индексирование",
            "Индексатор молотит в фоне: в начале каждого боя получаете Хелсчек 3.",
            Rarity.COMMON,
        )

    def on_combat_start(self, combat_manager):
        combat_manager.player.add_status("healthcheck", 3, combat_manager)
        combat_manager.add_log_message(
            f"[Реликвия] '{self.name}': Хелсчек 3!"
        )
