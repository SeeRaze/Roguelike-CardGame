...
def assign_enemy_ranks(enemies) -> list:
    """Расставить ВРАГОВ по 2D-сетке (РАНГ + ЛИНИЯ) — симметрия v1 (С48, §8/§9).

    РАНГ: ФРОНТ получает первую ПОЛОВИНУ группы (округление вверх), остальные —
    ТЫЛ. Параметр-фри, масштабируется: 1→1Ф · 2→1Ф/1Б · 3→2Ф/1Б · 4→2Ф/2Б. Даёт
    НЕПУСТОЙ фронт → перехват игрок→враг работает.
