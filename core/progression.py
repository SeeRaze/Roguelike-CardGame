...
LOCKED_RELICS: set = {
    ...
    # Синергия-реликвии (Блок 3, С65): все 10 — награда за прогресс (UNCOMMON+ доктрина,
    # вкл. 2 COMMON стихия-энейблера, как Кофемашина). Имена = class __name__ (relic_id).
...
# MVP-ачивки (С70 Этап 3, см. `core/achievements.py`) → конкретные гранты:
...
#
# Контент гл.1 — карты (С71): 6 КАРТ-ЯКОРЕЙ переведены casino → ачивки по критерию
# «якорь билда vs широта» (решение юзера). Якорь = payoff/keystone, вокруг которого
# собирают архетип → его хочется добыть ПРИЦЕЛЬНО (канал «Я ВЫБРАЛ»). Каждая ачивка
# выбивается на СТАРТОВЫХ инструментах (не требует залоченной карты) — предикат
# сигналит «ты готов к этому билду». См. core/achievements.py §контент-гл.1.
#   Параллелизм   → task_manager   (множитель Эхо ×2)
#   Накопилось    → version_control(удвоитель стихийных стаков)
#   Дамп ядра     → memory_dump    (decomp-payoff)
#   Дежавю        → echo_cascade   (Эхо-payoff ×2)
#   Зоопарк багов → tech_regression(радуга-стихий payoff)
#   На честном слове → prod_crutch (Стажёр-страховка от смерти)
ACHIEVEMENT_GRANT_CARDS:  set = {
    "steel_barricade", "boil", "final_deploy",
    # Контент гл.1 — карты-якоря (С71):
    "task_manager", "version_control", "memory_dump",
    "echo_cascade", "tech_regression", "prod_crutch",
}
