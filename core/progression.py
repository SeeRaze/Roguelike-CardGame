...
# ════════════════════════════════════════════════════════════════════════════
# ИСПЫТАНИЯ ПОД ЛЕГЕНДАРКИ (С72) — реликвии ТОЛЬКО за событие-испытание
# ════════════════════════════════════════════════════════════════════════════
# Двусторонние LEGENDARY-джокеры берутся НЕ рандом-дропом, а за осознанное
# событие-испытание ([[legendary-challenges]]). Ортогонально мете (Option A,
# решение юзера С72): казино/ачивка по-прежнему ОТКРЫВАЕТ легендарку «в принципе»
# (она остаётся в LOCKED_RELICS + casino/achievement-канале), но в ЗАБЕГЕ больше
# не выпадает рандомом — её выдаёт ТОЛЬКО развязка испытания (именной gain_relic).
# Здесь — id, исключаемые из рандом-выдачи (_pick_relic + gain_random_relic).
# Реликвии ОСТАЮТСЯ в ALL_RELICS: библиотека, мета-анлок и gain_relic живут.
# (Марш смерти — пилот — НЕ здесь: он вообще вне RELIC_POOL/LOCKED, изолирован.)
CHALLENGE_RELICS: set = {
    "ТочкаОтказа",     # Z2: испытание «Незаменимый» → «Точка отказа»
    "ДеплойВПятницу",  # Z3: испытание «Пятница, 17:00» → «Деплой в пятницу»
    "ЗероДаунтайм",    # Z4: испытание «Аптайм 99.99%» → «Зеро-даунтайм»
}

# Инвариант: challenge-реликвия обязана быть мета-залоченной (Option A —
# «мета открывает испытание»), иначе открывать её «в принципе» нечем.
assert CHALLENGE_RELICS <= LOCKED_RELICS, \
    "CHALLENGE_RELICS содержит id вне LOCKED_RELICS"


def is_challenge_relic(relic_id: str) -> bool:
    """True, если реликвия выдаётся ТОЛЬКО за испытание — исключена из рандом-
    дропа (_pick_relic, gain_random_relic). Мета-анлок и gain_relic не задеты."""
    return relic_id in CHALLENGE_RELICS
...
# Контент гл.1 — релики (С71): casino-heavy (решение юзера). Мощь EPIC/LEG остаётся
# рандом-кайфом казино; в ачивки — лишь ~маркерные движки с ярким milestone-предикатом
# (релики = пассивные снежные комья → им идут «прогресс-вехи»).
#   Карьерный рост → ПовышениеГрейда (+25% урон/босс) · Марафонец → Стрессоустойчивость.
ACHIEVEMENT_GRANT_RELICS: set = {
    "ДашбордМетрик", "Автодополнение", "ДеплойВПятницу",
