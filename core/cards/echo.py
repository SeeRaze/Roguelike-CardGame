...
# E5/E6: единственный путь ретриггера (Диспетчер задач тоже несёт EchoEffect);
# «Отзвук» (урон+Эхо 1) срезан как дубль Диспетчера.
#
# Дуэт:
#   «Ретрай»     — чистый энейблер: Эхо 2(3) → мульти-стак под ×3/×4.
...
class EchoEffect:
    ...
    SCALES_WITH_FORGE=False: Эхо — чистый МНОЖИТЕЛЬ урона (×N на следующие карты),
    не магнитуда. Линейный слой ковки (+δ/ур.) его НЕ трогает, иначе прокачка даёт
    рунэвей (ур.5 → Эхо 5 = ×6 на карту). Как у братьев Draw/Energy/Scry. Разовый
    апгрейд ур.1 (base→upgrade) остаётся — «Эхо 2(3)» = честный кап."""
...
def create_echo_resonance():
    """«Ретрай» — чистый энейблер: Эхо 2(3) за 1 энергию. Готовит следующую
    карту под тройное/четверное срабатывание."""
    return Card(
        name="Ретрай",
        cost=1,
        card_type="skill",
