...
class DisciplineBurstDamageEffect:
    ...
    Вне Дисциплины → просто base (множитель ×1). ЗАКРЕПЛЕНО С67 (капстоун): k=0.30/0.40
    (mult_per/upgrade_mult_per). Проба чувствительности: +0.10 k ≈ +1 к ceiling Воина →
    текущий держит «умеренный потолок» (грань «Соблюдай»); задирать k = роль Берсерка."""
