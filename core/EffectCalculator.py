class EffectCalculator:
    ...
    def calculate_damage(attacker, target, base_damage,
        ...
        # 3. ТОКСИЧНЫЙ МЕНЕДЖМЕНТ атакующего (С58) — мультипл. снижение урона: ×0.9 за
        # стак, пол 20% (анти-полный-ноль). Саботаж урона врага. Читается, не тратится.
        # (Поглотил фэнтези-weak: С59 консолидация в стихии — один инструмент среза урона.)
        if attacker.tox > 0:
        ...
        # 4. РАЗЛИТЫЙ КОФЕ цели (С58) — амплификатор вход. урона АДДИТИВНО: +20% за стак
        # (×(1 + 0.2·N)), без мультипл. взрыва. Усилитель/катализатор. Персист (не тратится).
        # (Поглотил фэнтези-vulnerable: С59 консолидация в стихии — один амп вход. урона.)
        if target.coffee > 0:
