...
# выживаемость, FP = чистый оффенс). ЗАКРЕПЛЕНО С67: проба чувствительности показала
# forge-reach ПЛОСКИМ при cost 30→100 — бот мрёт ДО акта 3, где компаунд Закалки и
# живёт, → гард к этой цене слеп. Калибруется ФИЛОМ плейтеста, не свипом.
TEMPER_HP_PCT          = 0.20       # +20% к текущему max_hp за одну Закалку (КАЛИБР. 39.3)
TEMPER_GOLD_COST       = 60         # БАЗОВАЯ цена Закалки в ЗОЛОТЕ. 60 = реальный ранний
                                    # выбор vs реликвия (золото ~23-95/бой в акте 1).
                                    # С68: цена больше НЕ плоская — растёт от числа покупок
                                    # за забег и этажа (см. temper_price). Плоская версия
                                    # позволяла бесконтрольный спам Закалки в магазине.
TEMPER_COST_PER_BUY    = 40         # +к цене за каждую УЖЕ купленную Закалку (забег-скоуп)
TEMPER_COST_PER_FLOOR  = 3          # +к цене за текущий этаж (продвижение по башне)
...
def temper_price(count: int, floor: int) -> int:
    """Цена СЛЕДУЮЩЕЙ Закалки (С68): база + надбавка за уже купленные Закалки в этом
    забеге + надбавка за этаж. Растущая цена душит спам Закалки в магазине и увязывает
    сток золота с продвижением по башне. ЧИСТАЯ функция (читает globals модуля → свип
    влияет без рассинхрона). count — сколько Закалок уже куплено за забег."""
    return (TEMPER_GOLD_COST
            + TEMPER_COST_PER_BUY * max(0, count)
            + TEMPER_COST_PER_FLOOR * max(0, floor))


def temper(player, gold_available: int, count: int = 0, floor: int = 1):
    ...
    С68: цена РАСТУЩАЯ (temper_price от count покупок и floor этажа), не плоская.

    ЧИСТАЯ функция (инвариант core/forge.py — без зависимости от gm): принимает
    доступное золото + счётчик покупок + этаж, возвращает (ok, gold_spent). Списание
    золота с кошелька И инкремент счётчика (gm.temper_count) — на вызывающем слое.
    Зеркало sharpen. Предполагает, что max_hp/hp у игрока уже инициализированы."""
    price = temper_price(count, floor)
